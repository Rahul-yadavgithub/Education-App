...
// Head roles
Userrouter.get("/headOfCollegeDashboard", isAuth, authorizeRole("headOfCollege"), (req, res) => {
  res.json({ success: true, message: "Welcome Head of College!", user: req.user });
});

Userrouter.get("/headOfDistrictDashboard", isAuth, authorizeRole("headOfDistrict"), (req, res) => {
  res.json({ success: true, message: "Welcome Head of District!", user: req.user });
});
